"use client";

import React from 'react';
import Image from 'next/image';
import {
  Mail,
  MapPin,
  Phone,
  ArrowUpRight,
  ShieldCheck,
  FileText
} from 'lucide-react';

// Custom Official SVG Icons for Brand Authenticity
const FacebookIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" className={props.className} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
  </svg>
);

const WhatsAppIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" className={props.className} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.746.953 3.71 1.458 5.704 1.459h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
);

const LinkedinIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" className={props.className} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
  </svg>
);

const data = {
  facebookLink: 'https://www.facebook.com/profile.php?id=100063562046259',
  whatsappLink: 'https://wa.me/2290161336548',
  linkedinLink: 'https://linkedin.com',
  services: {
    formation: '#services',
    accompagnement: '#services',
    provendes: '#produits',
    animaux: '#produits',
  },
  about: {
    history: '#about',
    vision: '#about',
    whyUs: '#pourquoi-nous',
    testimonials: '#témoignages',
  },
  help: {
    faqs: '#contact',
    support: '#contact',
    chat: 'https://wa.me/2290161336548',
  },
  contact: {
    email: 'contact@winagro.bj',
    phone: '+229 0161336548',
    address: 'Cotonou, République du Bénin',
  },
  company: {
    name: 'Win Agro',
    description:
      'Nous cultivons des élevages rentables et durables en transmettant les meilleures techniques biologiques adaptées à notre climat béninois.',
    logo: '/favicon-for-app/icon0.svg',
  },
};

const socialLinks = [
  { icon: FacebookIcon, label: 'Facebook', href: data.facebookLink },
  { icon: WhatsAppIcon, label: 'WhatsApp', href: data.whatsappLink },
  { icon: LinkedinIcon, label: 'LinkedIn', href: data.linkedinLink },
];

const aboutLinks = [
  { text: 'Notre Histoire', href: data.about.history },
  { text: 'Vision Biologique', href: data.about.vision },
  { text: 'Pourquoi Nous', href: data.about.whyUs },
  { text: 'Témoignages Réels', href: data.about.testimonials },
];

const serviceLinks = [
  { text: 'Formation Élevage', href: data.services.formation },
  { text: 'Accompagnement Terrain', href: data.services.accompagnement },
  { text: 'Nutrition (Provendes)', href: data.services.provendes },
  { text: 'Animaux Sélectionnés', href: data.services.animaux },
];

const helpfulLinks = [
  { text: 'Nous Contacter', href: data.help.faqs },
  { text: 'Support Urgent', href: data.help.support },
  { text: 'Discuter en Direct', href: data.help.chat, hasIndicator: true },
];

const contactInfo = [
  { icon: Mail, text: data.contact.email, isEmail: true },
  { icon: Phone, text: data.contact.phone, isPhone: true },
  { icon: MapPin, text: data.contact.address, isAddress: true },
];

export default function Footer4Col() {
  const currentYear = new Date().getFullYear();

  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    if (href.startsWith('#')) {
      e.preventDefault();
      const targetElement = document.querySelector(href);
      if (targetElement) {
        const offset = 80;
        const elementPosition = targetElement.getBoundingClientRect().top;
        const offsetPosition = elementPosition + window.pageYOffset - offset;
        window.scrollTo({
          top: offsetPosition,
          behavior: 'smooth',
        });
      }
    }
  };

  return (
    <footer className="bg-noir-vert text-gray-300 mt-24 w-full place-self-end rounded-t-[2.5rem] border-t-4 border-primary-green relative overflow-hidden">
      {/* Premium grain overlay for rich textures */}
      <div className="absolute inset-0 bg-grain opacity-[0.03] pointer-events-none" />

      <div className="mx-auto max-w-7xl px-6 pt-20 pb-8 sm:px-8 lg:px-12 lg:pt-24 relative z-10">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-3">
          
          {/* Column 1: Company Profile */}
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              <div className="relative w-16 h-16 rounded-full overflow-hidden border border-primary-green/30 bg-white logo-light-beam shadow-md flex items-center justify-center p-0.5">
                <img
                  src={data.company.logo || '/placeholder.svg'}
                  alt="Win Agro Logo"
                  className="h-14 w-14 object-contain rounded-full"
                />
              </div>
              <div className="flex flex-col">
                <span className="font-serif text-xl font-bold leading-tight text-white tracking-wide">
                  {data.company.name}
                </span>
                <span className="text-[9px] font-sans font-bold uppercase tracking-wider text-accent-yellow">
                  Agri Tech Solutions
                </span>
              </div>
            </div>

            <p className="text-gray-400 mt-6 max-w-md text-sm leading-relaxed text-left">
              {data.company.description}
            </p>

            <ul className="mt-8 flex gap-4 justify-start">
              {socialLinks.map(({ icon: Icon, label, href }) => (
                <li key={label}>
                  <a
                    href={href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 rounded-full bg-primary-green/20 hover:bg-primary-green text-white hover:text-white flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 shadow-md"
                  >
                    <span className="sr-only">{label}</span>
                    <Icon className="w-5 h-5" />
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Grid columns 2-5: Dynamic Links */}
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4 lg:col-span-2">
            
            {/* À Propos */}
            <div className="text-left">
              <p className="text-white font-serif text-base font-bold tracking-wider mb-6 border-b border-primary-green/20 pb-2">
                À Propos
              </p>
              <ul className="mt-6 space-y-4 text-sm font-sans">
                {aboutLinks.map(({ text, href }) => (
                  <li key={text}>
                    <a
                      className="text-gray-400 hover:text-accent-yellow transition-colors duration-200"
                      href={href}
                      onClick={(e) => handleLinkClick(e, href)}
                    >
                      {text}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Nos Services */}
            <div className="text-left">
              <p className="text-white font-serif text-base font-bold tracking-wider mb-6 border-b border-primary-green/20 pb-2">
                Services
              </p>
              <ul className="mt-6 space-y-4 text-sm font-sans">
                {serviceLinks.map(({ text, href }) => (
                  <li key={text}>
                    <a
                      className="text-gray-400 hover:text-accent-yellow transition-colors duration-200"
                      href={href}
                      onClick={(e) => handleLinkClick(e, href)}
                    >
                      {text}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Liens Utiles */}
            <div className="text-left">
              <p className="text-white font-serif text-base font-bold tracking-wider mb-6 border-b border-primary-green/20 pb-2">
                Utiles
              </p>
              <ul className="mt-6 space-y-4 text-sm font-sans">
                {helpfulLinks.map(({ text, href, hasIndicator }) => (
                  <li key={text}>
                    <a
                      href={href}
                      target={hasIndicator ? "_blank" : undefined}
                      rel={hasIndicator ? "noopener noreferrer" : undefined}
                      onClick={(e) => handleLinkClick(e, href)}
                      className={`${
                        hasIndicator
                          ? 'group flex items-center gap-1.5 justify-start hover:text-accent-yellow transition-colors'
                          : 'text-gray-400 hover:text-accent-yellow transition-colors'
                      }`}
                    >
                      <span className={`${hasIndicator ? 'text-gray-400 group-hover:text-accent-yellow' : ''} transition-colors`}>
                        {text}
                      </span>
                      {hasIndicator && (
                        <span className="relative flex size-2">
                          <span className="bg-primary-green absolute inline-flex h-full w-full animate-ping rounded-full opacity-75" />
                          <span className="bg-primary-green relative inline-flex size-2 rounded-full" />
                        </span>
                      )}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Contactez-nous */}
            <div className="text-left">
              <p className="text-white font-serif text-base font-bold tracking-wider mb-6 border-b border-primary-green/20 pb-2">
                Contact
              </p>
              <ul className="mt-6 space-y-4 text-sm font-sans">
                {contactInfo.map(({ icon: Icon, text, isAddress, isEmail, isPhone }) => (
                  <li key={text}>
                    <a
                      className="flex items-start gap-2 justify-start group hover:text-accent-yellow transition-colors duration-200"
                      href={
                        isEmail
                          ? `mailto:${text}`
                          : isPhone
                          ? `tel:${text.replace(/\s+/g, '')}`
                          : '#'
                      }
                      target={isAddress ? "_blank" : undefined}
                      rel={isAddress ? "noopener noreferrer" : undefined}
                    >
                      <Icon className="text-primary-green size-4 shrink-0 mt-0.5 transition-colors group-hover:text-accent-yellow" />
                      {isAddress ? (
                        <address className="text-gray-400 group-hover:text-accent-yellow -mt-0.5 flex-1 not-italic transition-colors">
                          {text}
                        </address>
                      ) : (
                        <span className="text-gray-400 group-hover:text-accent-yellow flex-1 transition-colors">
                          {text}
                        </span>
                      )}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

          </div>
        </div>

        {/* Legal & Authors Footer bar */}
        <div className="mt-16 border-t border-primary-green/10 pt-8 relative z-10">
          <div className="text-center sm:flex sm:justify-between sm:text-left flex-row-reverse items-center justify-between gap-4">
            
            {/* Designer credits */}
            <p className="text-xs text-gray-500 mt-4 sm:mt-0 font-sans flex gap-2 justify-center sm:justify-end">
              <span>Conçu avec excellence par</span>
              <a
                href="https://ketermarketing.com"
                target="_blank"
                rel="noopener noreferrer"
                className="text-accent-yellow hover:underline font-bold flex items-center gap-0.5"
              >
                Keter Marketing <ArrowUpRight className="w-3 h-3" />
              </a>
            </p>

            {/* Copyright & Legal links */}
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-center justify-center sm:justify-start text-xs text-gray-400 font-sans">
              <p>&copy; {currentYear} {data.company.name}. Tous droits réservés.</p>
              <div className="hidden sm:inline text-primary-green/30">|</div>
              <div className="flex gap-3">
                <a href="#contact" onClick={(e) => handleLinkClick(e, "#contact")} className="hover:text-accent-yellow transition-colors flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5" /> Confidentialité
                </a>
                <span className="text-primary-green/30">·</span>
                <a href="#contact" onClick={(e) => handleLinkClick(e, "#contact")} className="hover:text-accent-yellow transition-colors flex items-center gap-1">
                  <FileText className="w-3.5 h-3.5" /> Mentions Légales
                </a>
              </div>
            </div>

          </div>
        </div>
      </div>
    </footer>
  );
}
